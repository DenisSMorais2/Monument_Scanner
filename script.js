...
const mapGetDirectionsBtn = document.getElementById('mapGetDirectionsBtn');
        ...
        const marker = L.marker([monument.lat, monument.lng])
            ...
            .bindPopup(`
                <div class="text-center">
                    ...
                    <span class="text-sm">${monument.description.substring(0, 80)}...</span><br>
                    <span class="text-blue-600 font-bold">${monument.points} pontos</span>
                    ${isScanned ? '<br><span class="text-green-600">✓ Descoberto</span>' : ''}
                    <br><button onclick="showMapMonumentDetails(${monument.id})" class="mt-2 bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600">Ver Detalhes</button>
                </div>
            `);
...
function getUserLocation() {
    ...
    locateBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Localizando...';
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            userLocationMarker.bindPopup('📍 Sua localização atual').openPopup();
            map.setView([lat, lng], 16);
            
            locateBtn.innerHTML = '<i class="fas fa-check mr-2"></i>Localizado!';
            setTimeout(() => {
                locateBtn.innerHTML = '<i class="fas fa-location-crosshairs mr-2"></i>Minha Localização';
...
function showMonumentDetails(monument) {
    ...
    state.currentMonumentForRoute = monument;
...
function showLocationOnMap(monument) {
    ...
    if (map) {
        ...
        // Find and open the monument's popup
        const markerData = markers.find(m => m.monument.id === monument.id);
        if (markerData) {
...
function showMapMonumentDetails(monumentId) {
    ...
    if (isScanned) {
        mapMonumentStatus.innerHTML = `
            ...
            <p class="text-xs text-gray-600">Descoberto</p>
        `;
        mapMonumentStatus.className = 'bg-green-50 p-3 rounded-lg text-center';
    } else {
        mapMonumentStatus.innerHTML = `
            <div class="text-lg font-bold text-gray-600">
                <i class="fas fa-lock"></i>
            </div>
            <p class="text-xs text-gray-600">Não Descoberto</p>
        `;
        mapMonumentStatus.className = 'bg-gray-50 p-3 rounded-lg text-center';
    }
...
window.showMapMonumentDetails = showMapMonumentDetails;
...
// Monument modal event listeners
showLocationBtn.addEventListener('click', () => {
    if (state.currentMonumentForRoute) {
        showLocationOnMap(state.currentMonumentForRoute);
    }
});

getDirectionsBtn.addEventListener('click', () => {
    if (state.currentMonumentForRoute) {
        getDirections(state.currentMonumentForRoute);
    }
});

mapGetDirectionsBtn.addEventListener('click', () => {
    if (state.currentMonumentForRoute) {
        getDirections(state.currentMonumentForRoute);
    }
});
